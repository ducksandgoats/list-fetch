    ...
    try {
      ...
      if(method === 'HEAD'){
        ...
        if(!body){
          ...
          if (mid.mainQuery) {
            if(JSON.parse(reqHeaders['x-update']) || JSON.parse(searchParams.has('x-update'))){
              ...
              return {statusCode: 200, headers: {'Content-Length': String(torrentData.length), 'Content-Type': mainRes, 'X-Infohash': torrentData.infohash, 'X-Title': torrentData.title}, data: mainReq ? [`<html><head><title>${torrentData.name}</title></head><body><div><p>infohash: ${torrentData.infohash}</p><p>title: ${torrentData.title}</p></div></body></html>`] : [JSON.stringify(torrentData.files.map(data => {return {file: data.name, link: `bt://${torrentData.infohash}${data.urlPath}`}}))]}
            }
          } else {
        ...
        if (mid.mainQuery) {
          ...
          return {statusCode: 200, headers: {'Content-Type': mainRes}, data: mainReq ? [`<html><head><title>${mid.mainHost}${mid.mainPath}</title></head><body><div><p>${torrentData.type}: ${torrentData.id}</p><p>path: ${torrentData.path}</p><p>link: bt://${torrentData.id}/</p></div></body></html>`] : [JSON.stringify({id: torrentData.id, [torrentData.type]: torrentData.id, path: torrentData.path, link: `bt://${torrentData.id}/`})]}
        }
      } else {
